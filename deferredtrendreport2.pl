...
my $sqlcountone = 'select count(*) from bugs where product_id IN (2)
							   and creation_ts >= "2012-07-27" and creation_ts <= "2012-08-03" and (cf_target_release NOT IN("2.0") and version IN ("1.1")
							  and cf_target_release NOT IN("1.1.0") and cf_target_release NOT IN("1.0.1.3")) and bug_severity = "S1 - DU/DL"';
							
							
#Query for severity two DEFEERRED bugs	
my $sqlcounttwo = 'select count(*) from bugs where product_id IN (2)
							   and creation_ts >= "2012-07-27" and creation_ts <= "2012-08-03" and (cf_target_release NOT IN("2.0") and version IN ("1.1")
							  and cf_target_release NOT IN("1.1.0") and cf_target_release NOT IN("1.0.1.3")) and bug_severity = "S2 - Major (Crash/Broken Feature)"';
#Query for severity three DEFERRED bugs	
my $sqlcountthree = 'select count(*) from bugs where product_id IN (2)
							   and creation_ts >= "2012-07-27" and creation_ts <= "2012-08-03" and (cf_target_release NOT IN("2.0") and version IN ("1.1")
							  and cf_target_release NOT IN("1.1.0") and cf_target_release NOT IN("1.0.1.3")) and bug_severity = "S3 - Minor (Cosmetic)"';	
#Query for severity four DEFERRED bugs		
my $sqlcountfour = 'select count(*) from bugs where product_id IN (2)
							   and creation_ts >= "2012-07-27" and creation_ts <= "2012-08-03" and (cf_target_release NOT IN("2.0") and version IN ("1.1")
							  and cf_target_release NOT IN("1.1.0") and cf_target_release NOT IN("1.0.1.3")) and bug_severity = "S4 - Enhancement Request"';	
#Query for severity five DEFERRED bugs		
my $sqlcountfive = 'select count(*) from bugs where product_id IN (2)
							   and creation_ts >= "2012-07-27" and creation_ts <= "2012-08-03" and (cf_target_release NOT IN("2.0") and version IN ("1.1")
							  and cf_target_release NOT IN("1.1.0") and cf_target_release NOT IN("1.0.1.3")) and bug_severity = "S5 - Task Tracking"';		
#create a hash


my $sqltotal = 'select count(*) from bugs where product_id IN (2)
							  and creation_ts >= "2012-07-27" and creation_ts <= "2012-08-03" and (cf_target_release NOT IN("2.0") and version IN ("1.1")
							  and cf_target_release NOT IN("1.1.0") and cf_target_release NOT IN("1.0.1.3"))';	
...
write2file( 'deferredtrend2.csv', $content);
